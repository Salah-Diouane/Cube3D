...
static int  is_wall(t_data *dt, t_point *p, int x_d, int y_d)
{
    ...
    if ((p->x < 0) || (p->y < 0) || ((dt->grd_wd * \
        dt->s_map->width) < p->x) || ((dt->grd_ht * dt->s_map->height) < p->y))
        return (1);
    else if (dt->s_map->map[j][i] == '1')
        return (1);
    else if (dt->s_map->map[j][i] == 'D')
    {
...
static double hor_inter(t_data *data, t_point *inter, double angle)
{
    ...
    while ((0 < inter->y) && (inter->y < (grd_ht * data->s_map->height)))
    {
...
static double ver_inter(t_data *data, t_point *inter, double angle)
{
    ...
    while ((0 < inter->x) && (inter->x < (grd_wd * data->s_map->width)))
    {
