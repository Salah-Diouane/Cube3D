...
/*   Updated: 2024/08/07 14:38:16 by sdiouane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int  render_pixel(t_data *dt, int *ref, int *ind, int *grd)
{
    ...
    ind[0]++;
    if ((0 < ind[1]) && (ind[1] < (grd[1] * dt->s_map->height)) && \
        (0 < ind[0]) && (ind[0] < (grd[0] * dt->s_map->width)))
    {
        tmp = dt->s_map->map[(int)(ind[1] / grd[1])][(int)\
            (ind[0] / grd[0])];
        if (tmp == '1')
...
int ft_mini_map(t_data *data)
{
    ...
    if (data && data->s_map->map && data->map_img)
    {
        ...
        {
            if (data->s_map->map && data->map_img)
